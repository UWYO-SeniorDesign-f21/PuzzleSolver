...
class PuzzleSolver:
    ...
    def doGeneration(self):
        ...
        selection = select_all(self.solutions, self.gen_size//2, 2, include=include)
        self.solutions = None
        gc.collect()
        ...
        self.solutions = [solution.mutate(self.getMutationRate(solution)) for solution in new_solutions]
        ...
        diversities = sorted([solution.similarity_score for solution in self.solutions])

        self.updateSimilarityScore(self.solutions)

        print(f'solution len edges: {[len(solution.edges) for solution in self.solutions[:10]]}')
        ...
        print(f'top 10 scores diversities: {[solution.similarity_score for solution in self.solutions[:10]]}')
        print(f'top 10 scores mutation rates: {[self.getMutationRate(solution) for solution in self.solutions[:10]]}')
        print(f'top 10 scores expected # mutations: {[1 / (1 - self.getMutationRate(solution)) - 1 for solution in self.solutions[:10]]}')
    ...
    def doGen0(self, just_sides, include_edges):
        ...
        print(f'top 10 gen {0} scores: {scores[:10]},\n top 10 unique: {scores2[:10]}')
        print(f'average score: {sum(scores) // len(scores)}, num varieties: {len(scores2)}\n\n')
...
def getDistDict(pieces, weight_dist=100, weight_color=100, weight_color_hist=100, weight_length_diff=100):
    ...
    print(max_dist_diff, min_dist_diff, max_color_diff, min_color_diff, max_color_diff_hist, min_color_diff_hist, max_corner_diff, min_corner_diff)

    num_middle_pieces = len([piece for piece in pieces if piece.type == 'middle'])
    ...
    for piece1 in pieces:
        for edge1 in range(4):
            sorted_dists[(piece1, edge1)] = [((None, None), float('inf')) for _ in range(len(pieces) // 4)]
                ...
                for edge2 in range(4):
                    ...
                    if dist < float('inf'):
                        ...
                        if dist < sorted_dists[(piece1, edge1)][-1][1]:
                            ...
                            sorted_dists[(piece1, edge1)] = sorted(sorted_dists[(piece1, edge1)], key=lambda x:x[1])
                        if dist < sorted_dists[(piece2, edge2)][-1][1]:
                            sorted_dists[(piece2, edge2)][-1] = ((piece1, edge1), dist)
                            sorted_dists[(piece2, edge2)] = sorted(sorted_dists[(piece2, edge2)], key=lambda x:x[1])
    cutoff = best_sides[-1] + 1

    for piece1 in pieces:
        for edge1 in range(4):
            sorted_dists[(piece1, edge1)] = [entry for entry in sorted_dists[(piece1, edge1)] if entry[1] != float('inf')]
